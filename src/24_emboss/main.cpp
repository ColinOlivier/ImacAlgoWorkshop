...
glm::vec3 getPixelEmboss(sil::Image &image, glm::ivec2 pixelPosition);
...
int main()
{
    ...
    for (size_t x = 0; x < input_image.width(); x++)
    {
        ...
        {
            output_image.pixel(x, y) = getPixelEmboss(input_image, glm::ivec2{x, y});
        }
    }

    output_image.save("output/emboss.png");
}

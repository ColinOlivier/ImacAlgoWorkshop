...
glm::vec3 getPixelOutline(sil::Image &image, glm::ivec2 pixelPosition);
...
int main()
{
    ...
    for (size_t x = 0; x < input_image.width(); x++)
    {
        ...
        {
            output_image.pixel(x, y) = getPixelOutline(input_image, glm::ivec2{x, y});
        }
    }

    output_image.save("output/outline.png");
}

glm::vec3 getPixelOutline(sil::Image &image, glm::ivec2 pixelPosition)
{
    glm::mat3 kernel{-1.f, -1.f, -1.f, -1.f, 8.f, -1.f, -1.f, -1.f, -1.f};
